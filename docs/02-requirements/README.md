...
| **AS-007** | Recovery Time Objective (RTO) and Recovery Point Objective (RPO) are derived from industry average attack frequency (~1968 threats every week per organisation) | [SentinelOne - Key Cyber Security Statistics for 2026](https://www.sentinelone.com/cybersecurity-101/cybersecurity/cyber-security-statistics/) |
...
| **AS-010** | File Analysis retained storage is negligible relative to Netflow storage. Based on global average post-spam-filter inbound email volume (~40 emails/endpoint/day), malware infection rate (~0.1%) and average malicious payload (~2 MB), infected file retention at XLarge is estimated at ~72 GB over 3 months, that accounts to ~1% of the Netflow capacity. FTP file transfer volume is considered negligible in modern enterprise networks | Engineering estimate, email volume from [EmailToolTester / Radicati 2024](https://www.emailtooltester.com/en/blog/email-usage-statistics/), infection rate from [Barracuda Email Threats Report 2026](https://www.barracuda.com/reports/2026-email-threats-report), FTP decline from [Censys FTP Exposure Brief 2026](https://censys.com/blog/ftp-exposure-brief/) |
...
| **AS-012** | C&C logs are retained for detections only. Full-score logging would support model drift detection and threshold calibration but is deferred as an MLOps concern out of scope (AS-003). NF metadata (retained 3 months, *[FR-NF-004](2.1-logical-requirements.md#network-forensics-nf)*) enables retrospective re-scoring within the retention window. | Architectural decision |
| **AS-013** | C&C log storage is negligible. Only suspected C&C activity above threshold is logged. Even assuming 10,000 detections/day at XLarge, annual log storage is under 1 GB, less than 0.01% of NF storage. | Engineering estimate |
