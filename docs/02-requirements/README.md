...
| **AS-012** | Only suspected C&C activity (above threshold) is logged. Logging all analysis results would duplicate NF metadata at significantly higher storage cost (~52 TB/year at XLarge) with no additional forensic value. | Architectural decision |
